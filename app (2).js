...
// Pull the current batter from any of the places ESPN puts them.
// Returns { name, stats } or null.
function resolveBatter(situation, summary) {
  // Pattern 1: situation.batter.athlete is fully expanded
  const b = situation?.batter;
  if (b?.athlete?.displayName) {
    return { name: b.athlete.displayName, stats: b.summary || '' };
  }
  if (b?.playerSummary) return { name: b.playerSummary, stats: '' };

  // Pattern 2: most recent play's batter
  const plays = summary?.plays || [];
  for (let i = plays.length - 1; i >= 0; i--) {
    const p = plays[i];
    const name = p?.participants?.find(x => /batter/i.test(x.type || ''))?.athlete?.displayName
              || p?.batter?.athlete?.displayName
              || p?.atBatPlayer?.fullName;
    if (name) return { name, stats: '' };
  }
  return null;
}

function resolvePitcher(situation, summary) {
  const p = situation?.pitcher;
  if (p?.athlete?.displayName) {
    return { name: p.athlete.displayName, stats: p.summary || '' };
  }
  if (p?.playerSummary) return { name: p.playerSummary, stats: '' };

  const plays = summary?.plays || [];
  for (let i = plays.length - 1; i >= 0; i--) {
    const pl = plays[i];
    const name = pl?.participants?.find(x => /pitcher/i.test(x.type || ''))?.athlete?.displayName
              || pl?.pitcher?.athlete?.displayName;
    if (name) return { name, stats: '' };
  }
  return null;
}
...
function renderGame(competition, summary, state) {
  ...
  if (isLive && situation) {
    ...
    // Runner summary. ESPN often returns onFirst/onSecond/onThird as booleans
    // (true/false), not as player objects. Try to find names from nested objects
    // first, then fall back to a clean "Runners on 1st, 2nd" style summary.
    const runnerSlots = [
      { base: '1st', flag: situation.onFirst },
      { base: '2nd', flag: situation.onSecond },
      { base: '3rd', flag: situation.onThird },
    ];
    const namedRunners = [];
    const occupiedBases = [];
    for (const r of runnerSlots) {
      if (!r.flag) continue;
      occupiedBases.push(r.base);
      const name = (typeof r.flag === 'object')
        ? (r.flag.playerSummary || r.flag.athlete?.displayName || r.flag.shortName)
        : null;
      if (name) namedRunners.push(`${r.base}: ${name}`);
    }
    const runnersEl = $('runners-list');
    if (runnersEl) {
      if (namedRunners.length) {
        runnersEl.hidden = false;
        runnersEl.textContent = namedRunners.join('  •  ');
      } else if (occupiedBases.length) {
        runnersEl.hidden = false;
        runnersEl.textContent = `Runners on ${occupiedBases.join(', ')}`;
      } else {
        runnersEl.hidden = false;
        runnersEl.textContent = 'Bases empty';
      }
    }

    // Batter / Pitcher — try situation first, then boxscore (some games only have it there)
    const batterInfo = resolveBatter(situation, summary);
    if (batterInfo) {
      $('batter-name').textContent = batterInfo.name;
      $('batter-stats').textContent = batterInfo.stats || '';
    } else {
      $('batter-name').textContent = '—';
      $('batter-stats').textContent = '';
    }
    const pitcherInfo = resolvePitcher(situation, summary);
    if (pitcherInfo) {
      $('pitcher-name').textContent = pitcherInfo.name;
      $('pitcher-stats').textContent = pitcherInfo.stats || '';
    } else {
  ...
  if (state !== 'pre') {
    ...
    renderLinescore(home, away, summary?.plays || []);
  } else {
...
function renderLinescore(home, away, plays) {
  // If linescores arrays are empty/missing, derive from play-by-play.
  const homeLS = (home.linescores?.length ? home.linescores : derivedLinescores(plays, false));
  const awayLS = (away.linescores?.length ? away.linescores : derivedLinescores(plays, true));
  const homeAug = { ...home, linescores: homeLS };
  const awayAug = { ...away, linescores: awayLS };

  // Use the larger of: home linescore length, away linescore length, or 9 innings minimum
  const innings = Math.max(homeLS?.length || 0, awayLS?.length || 0, 9);
  const head = ['<th></th>'];
  for (let i = 1; i <= innings; i++) head.push(`<th>${i}</th>`);
  head.push('<th>R</th><th>H</th><th>E</th>');
  $('linescore-head').innerHTML = head.join('');

  // Helper: get the value for inning N (1-indexed) from a team's linescores array.
  const inningVal = (team, inning) => {
    const ls = team.linescores;
    if (!Array.isArray(ls) || !ls.length) return '·';
    let entry = ls.find(l => l && (l.period === inning || l.period === String(inning)));
    if (!entry) entry = ls[inning - 1];
    if (entry === null || entry === undefined) return '·';
    if (typeof entry === 'object') {
      if (entry.displayValue !== undefined && entry.displayValue !== null) return entry.displayValue;
  ...
  const rowFor = (team) => {
    ...
    for (let i = 1; i <= innings; i++) {
      cells.push(`<td>${inningVal(team, i)}</td>`);
    }
    const stats = team.statistics || [];
    const stat = (n) => stats.find(s => s.name === n || s.abbreviation === n)?.displayValue ?? '—';
  ...
  $('linescore-body').innerHTML = rowFor(awayAug) + rowFor(homeAug);
}

// Build runs-per-inning from play-by-play when linescores aren't on the team object.
// `isAwayHalf` true = top-half plays (away batting), false = bottom-half (home batting)
function derivedLinescores(plays, isAwayHalf) {
  ...
  for (const p of plays) {
    ...
    const isTop = halfRaw.includes('top');
    if (isTop !== isAwayHalf) continue;
    const runs = p.scoringPlay ? (p.scoreValue || 1) : 0;
    byInning.set(inning, (byInning.get(inning) || 0) + runs);
  }
  if (!byInning.size) return [];
  const maxInning = Math.max(...byInning.keys());
  const out = [];
  for (let i = 1; i <= maxInning; i++) {
    out.push({ period: i, displayValue: String(byInning.get(i) ?? 0), value: byInning.get(i) ?? 0 });
  }
  return out;
}

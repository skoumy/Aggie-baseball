...
$(‘card-loading’).innerHTML = `<div style="text-align:left;max-width:480px;margin:0 auto;"> <div style="color:#ef4444;font-weight:600;margin-bottom:8px;">⚠ Couldn't load data</div> <div style="color:#a08a78;font-size:12px;line-height:1.5;font-family:'Roboto Mono',monospace;word-break:break-all;">${msg}</div> <div style="color:#7a6557;font-size:11px;margin-top:12px;"> Check the browser console (F12 / ⌥⌘I) for details. </div> </div>`;
}
...
return `<li class="pbp-item${scoring ? ' scoring' : ''}"> <span class="pbp-inning">${half} ${periodNum}</span> <span class="pbp-text">${text}</span> </li>`;
}).join(’’);
}
...
: ‘’;
return `<details class="inning-block" ${b === sorted[0] ? 'open' : ''}> <summary> <span class="inning-half">${halfLabel} ${b.inning}</span> <span class="inning-bat">${battingAbbr} batting</span> <span class="inning-stats">${b.runs} R · ${b.hits} H</span> </summary> ${scoringText} <div class="inning-last">${(summary || '').replace(/</g, '&lt;')}</div> </details>`;
}).join(’’);
}
...
return `<tr> <td class="bx-name">${name}${pos ? ` <span class="bx-pos">${pos}</span>` : ''}${note}</td> ${cells} </tr>`;
}).join(’’);
